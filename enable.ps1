...
try{
    if(-Not($dryRun -eq $True)) {
        ...
        Write-Verbose "Enabling account $($account.Username) ($($account.UserGUID))"
        ...
        Write-Information "Successfully enabled account $($aRef.Username) ($($aRef.UserGUID))"

        $success = $true;
        $auditLogs.Add([PSCustomObject]@{
            Action  = "EnableAccount"
            Message = "Enabled account $($aRef.Username) ($($aRef.UserGUID))";
            IsError = $false;
        }); 
    }
}catch{
    if($error[0].Exception -like "*404*"){
        $auditLogs.Add([PSCustomObject]@{
            Action = "EnableAccount"
            Message = "Error enabling account $($account.Username): User not found. Check if the user still exists in HelloID and validate the aRef."
            IsError = $True
        });
        Write-Warning $_;
    }  
    elseif($error[0].Exception -like "*401*"){
        $auditLogs.Add([PSCustomObject]@{
            Action = "EnableAccount"
            Message = "Error enabling account $($account.Username): $($_). Check configuration settings and/or IP restrictions."
            IsError = $True
        });
        Write-Warning $_;
    }
        ...
        $auditLogs.Add([PSCustomObject]@{
            Action = "EnableAccount"
            Message = "Error enabling account $($account.Username): $($_)"
            IsError = $True
        });
        Write-Warning $_;
    }
