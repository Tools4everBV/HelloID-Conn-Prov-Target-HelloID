#################################################
# HelloID-Conn-Prov-Target-HelloID-Disable
# PowerShell V2
#################################################
...
try {
    ...
    switch ($action) {
        ...
        "NotFound" {
            $auditMessage = "Skipped updating account with AccountReference: $($outputContext.AccountReference | ConvertTo-Json). Reason: No No account found where [$($correlationField)] = [$($correlationValue)]. Possibly indicating that it could be deleted, or the account is not correlated."
